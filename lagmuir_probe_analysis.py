...
##################################
# %% Constants
from scipy.constants import c as _C, mu_0 as _MU_0, e as _E, m_e as _M_E # k as k_b, pi, , , h, hbar, G, g, , R, N_A, k as k_b, m_e, m_p, m_n
...
def _smooth_IV_data_v1(IV, smooth_width_in_volts=3, plot=False):
...
def plot_IV_data(IV, 
	...
	if plot_deriv is True:
		...
		dIdV = _xr.DataArray(_np.gradient(IV), 
					   coords=IV.coords)
...
def _polyFitData(da,	
				...
				verbose=False):
	""" 
	Polynomial fit function.  This is effectively a wrapper for numpy.poly1d()
...
##################################
# %% Plasma parameters
def _debye_length(density, temperature_in_eV):
	"""
...
def _plasma_frequency_from_density(density,  plot=False):
	"""
	...
	"""
	mass = _M_E
	frequency = _xr.DataArray(_np.sqrt(density * _E**2 / (mass * _EP_0)) * 1/(2 * _np.pi),
							  dims='n',
							  coords=[density],
...
def _density_from_frequency(frequency, plot=False):
	"""
	...
	"""
	mass = _M_E
	density = (2 * _np.pi * frequency)**2 * mass * _EP_0 / _E**2

	return density


def _plasma_frequency(n0, mass=_M_E):
	""" Calculates the plasma frequency in Hz """
	mass = _M_E
	return _np.sqrt(n0 * _E**2 / (mass * _EP_0)) * 1/(2 * _np.pi)


##################################
# %% Langmuir analysis subfunctions

def _calc_electron_density(I_electron, V_plasma, temperature_in_eV, probe_area):
	""" calculates electron density """
	I_e_sat = float(I_electron.interp(V=V_plasma))
	n_e = (I_e_sat / (_E * probe_area)) * _np.sqrt(2 * _np.pi * _M_E / (temperature_in_eV * _EV))
	return n_e


def _calc_Vp_from_theory(temperature_in_eV, V_float, ion_mass):
	return _np.log(_np.sqrt(ion_mass / (2 * _np.pi * _M_E))) * temperature_in_eV + V_float
	

def _calc_Vp_from_deriv_v1(I_electron_smoothed, V_float, plot=False):
	""" calculates V_plasma from the derivative of the smoothed V_float """
	dIdV = _xr.DataArray(_np.gradient(I_electron_smoothed), dims=I_electron_smoothed.dims, coords=I_electron_smoothed.coords)
...
def _calc_temp_in_eV_v1(exp_fit_coeff_with_volt_units):
	"""
	...
	"""
	return _E * exp_fit_coeff_with_volt_units / _EV
...
def _calc_temp_from_exp_fit_v2(I_electron, V_float, V_plasma, dV, expFitGuess, plot=False):
	""" Calculates temperature by fitting an exponential to the electron current """
	
	exp_fit_bounds = [V_float, (V_plasma - V_float)/1.5 + V_float]
	Ie_windowed = I_electron[(I_electron.V <= exp_fit_bounds[1]) & (I_electron.V >= exp_fit_bounds[0])]
	if len(Ie_windowed) == 2:
		print("Warning: 3 points minimum are required between V_float and V_plasma.  2 found.  Expaninding window to include a 3rd point.  ")
	...
	log_fit, ffit, _, _ = _polyFitData(ln_Ie, order=1, plot=False, verbose=False)
	temperature_in_eV = 1 / ffit.coeffs[0]
	log_fit = _np.exp(log_fit)
	
	return temperature_in_eV, log_fit


def _find_V_float_v1(IV, smooth_width_in_volts=1.0, pre_filter=True, plot=False):
	""" Finds V_float by interporloating V(I)=0 """
	
	I_min = _np.abs(float(IV.min()))
	
	if pre_filter is True:
		IV = _smooth_IV_data_v1(IV, plot=False, smooth_width_in_volts=smooth_width_in_volts)
		
	VI = _xr.DataArray(IV.V.data, dims='I', coords=[IV.data])
	V_float = VI.where((VI.I>=-I_min) & (VI.I<=I_min)).interp(I=0)
...
def IV_sweep_analysis(	IV, 
						probe_area_m2, 
						ion_mass, 
						expFitGuess=(6.0, 100.0, -5.0), # (a, b, c) where  a * _np.exp(x / b) + c
						smooth_width_in_volts=2.0,
	...
	expFitGuess : tuple of three floats
		Used for the initial fit of the temperature.  (a, b, c) where  a * _np.exp(x / b) + c
	smooth_width_in_volts : float
		Used for smoothing the IV data before various steps.  The value is sigma in a gaussian smoothing function and therefore has units in Volts.  
	temperature_in_eV_guess : float
		Guess at temperature.  Units in eV. The default is 2.0.
	plot : bool
		Optionally generates a plot of the results.  
	V_lim : list of two floats (or empty list)
	...
	calc_Vp_from_deriv : bool, optional
		There are two methods for calculating V_plasma.  If this value is True, it uses the derivative of IV to calculate V_plasma.  Otherwise, it uses the theory of V_plasma. Note that the derivative method often fails (for various reasons), and the equation method is more robust (i.e. setting this bool to False).
	Vp_guess : float of None
		The initial guess value for V_plasma.  If None, temperature_in_eV_guess is used to predict V_p
	...
	## (optional) trim voltage range
	if type(V_lim) is list and len(V_lim) == 2:
			IV = IV.where((IV.V >= V_lim[0]) & (IV.V <= V_lim[1])).dropna('V')
			
	## misc
	dV = _np.mean(IV.V[1:].values - IV.V[:-1].values)

	## step 2 - Calculate V_float
	V_float = _find_V_float_v1(IV, smooth_width_in_volts=smooth_width_in_volts, plot=plot_intermediate_steps)

	## step 3 - linear fit to the ion saturation current
	I_windowed_for_isat = IV[IV.V < (V_float - IV.V[0].data) / 1.25 + IV.V[0].data]
	I_ion_fit, I_ion_fit_func, _, _ = _polyFitData(I_windowed_for_isat, order=1, plot=plot_intermediate_steps)

	## step 4 - subtract ion saturation linear fit from I(V) to get electron current
	I_electron = IV.copy() - I_ion_fit_func(IV.V)
	I_electron_smoothed = _smooth_IV_data_v1(I_electron, smooth_width_in_volts=smooth_width_in_volts, plot=plot_intermediate_steps)

	## step 5 - guess at V_plasma
	if type(Vp_guess) is not type(None):
		V_plasma = Vp_guess
	else:
		V_plasma = _calc_Vp_from_theory(temperature_in_eV_guess, V_float, ion_mass, )	

	## step 6 - iteratively solve for V_plasma and Te
	for i in range(10):
		
		## Calc kTe/e (temperature with units in eV)
		temperature_in_eV, log_fit = _calc_temp_from_exp_fit_v2(I_electron, V_float=V_float, V_plasma=V_plasma, dV=dV, expFitGuess=expFitGuess, plot=plot_intermediate_steps)
		 			
		## Calc Vp
		if calc_Vp_from_deriv is True:
			V_plasma = _calc_Vp_from_deriv_v1(I_electron_smoothed, V_float, plot=plot_intermediate_steps)
		else:
	...
	## step 7 - electron density
	n_e = _calc_electron_density(I_electron, V_plasma, temperature_in_eV, probe_area=probe_area_m2, )
	if verbose:
		print('Electron density: %.3e m^-3' % n_e)

	## step 8 - preliminary ion density
	I_ion_sat = float(I_ion_fit_func(V_plasma))
	n_i = float(-(_np.exp(0.5) * I_ion_sat / (_E * probe_area_m2) * _np.sqrt(ion_mass / (temperature_in_eV * _EV))))
	if verbose:
		print('Approx ion. sat: %.3e A' % I_ion_sat)
		print('Ion density: %.3e m^-3' % n_i)
		
	## step 9 - iterate to calculate a better ion density
	# This step has not been implemented yet.  
		
	## misc final calculations
	debye = _debye_length(n_e, temperature_in_eV)
	if verbose:
		print('Debye length = %.2e m' % (debye))

	radius = _np.sqrt(probe_area_m2 / _np.pi) ## assuming circular
	if verbose:
		print('probe radius = %.2e m' % radius)

	if plot is True:
		linthresh = 10 ** _np.floor(_np.log10(_np.abs(I_ion_sat)))
		
		fig, ax = plot_IV_data(IV, 
			   label='IV data', 
			   marker='x', 
			   linestyle='',
			   linthresh=linthresh,
		...
		plot_IV_data(I_ion_fit.where(I_ion_fit.V<=V_float), 
			   fig=fig, 
			   ax=ax, 
			   label='Ion current fit', 
			   linewidth=2,
			   linthresh=linthresh,
			   plot_deriv=False,
			   )
		plot_IV_data(I_electron, 
			   fig=fig, 
			   ax=ax, 
			   ...
			   linthresh=linthresh,
			   plot_deriv=False,)
		plot_IV_data(log_fit, 
			   fig=fig, 
			   ax=ax, 
			   label='Electron current fit', 
			   linewidth=2,
			   linthresh=linthresh,
			   plot_deriv=False,)
		for a in ax:
	...
	return temperature_in_eV, n_e, V_float, V_plasma
